...
bst2a <- xgb.train(params = param, data = xgtrain, feval = evalgini, nround=num_rounds, print.every.n = 100, 
                   watchlist=watchlist, early.stop.round = pp5, maximize = TRUE)
preds2a <- predict(bst2a,xgtest)
preds2a_val <- predict(bst2a,xgval)
NormalizedGini(as.vector(train_y[1:offset]), as.vector(preds2a_val))
# Output submission
pred2a_df = data.frame(Id = test_Id, Hazard= preds2a)
write.table(pred2a_df, file = 'xgboost_2a.csv', row.names = F, col.names = T, sep = ",", quote = F)
...
bst2b <- xgb.train(params = param, data = xgtrain, nround=10000, print.every.n = 100, 
                   watchlist=watchlist, early.stop.round = 100, maximize = FALSE)
preds2b <- predict(bst2b,xgtest)
preds2b_val <- predict(bst2b,xgval)
NormalizedGini(as.vector(train_y[1:offset]), as.vector(preds2b_val))
# Output submission
pred2b_df = data.frame(Id = test_Id, Hazard= preds2b)
write.table(pred2b_df, file = 'xgboost_2b.csv', row.names = F, col.names = T, sep = ",", quote = F)
